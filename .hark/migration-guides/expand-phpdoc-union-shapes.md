...
This does not change accepted requests or runtime behavior. However, PHPStan and IDEs may now report arrays whose keys or values do not match the documented parameter shape. Update reported arguments to use the fields and value types shown in the method's PHPDoc. If a diagnostic is caused by an imprecisely inferred local variable, add an accurate array-shape annotation to that variable.
