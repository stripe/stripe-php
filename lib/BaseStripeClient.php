...
class BaseStripeClient implements StripeClientInterface, StripeStreamingClientInterface
{
    ...
    public function parseEventNotification($payload, $sigHeader, $secret, $tolerance = Webhook::DEFAULT_TOLERANCE)
    {
        ...
        // Rewrite the type in the JSON payload BEFORE fromJson so the type
        // registry resolves the correct EventNotification subclass.
        $parsed = json_decode($eventData, true);
        $evtType = $parsed['type'] ?? '';
        $relatedType = $parsed['related_object']['type'] ?? null;
        if (
            is_string($evtType) &&
            str_starts_with($evtType, 'v2.extend.objects.object_record') &&
            is_string($relatedType)
        ) {
            $parsed['type'] = str_replace(
                'v2.extend.objects.object_record',
                $relatedType,
                $evtType
            );
            $eventData = json_encode($parsed);
        }
